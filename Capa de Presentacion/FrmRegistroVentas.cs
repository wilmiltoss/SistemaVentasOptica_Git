...
namespace Capa_de_Presentacion
{
    public partial class FrmRegistroVentas : DevComponents.DotNetBar.Metro.MetroForm
    {
        ...
        {
            FrmListadoClienteVenta C = new FrmListadoClienteVenta();
            C.Show();
        }
        private void FrmVentas_Activated(object sender, EventArgs e)
        {
            //txtIdProducto.Text = Program.IdCliente+"";
            txtNroDocumento.Text = Program.CI;
            txtDatos.Text = Program.Apellidos + ", " + Program.Nombres;
            txtIdProducto.Text = Program.IdArticulo + "";
        ...
        {
            FrmListadoProductoVenta P = new FrmListadoProductoVenta();
            P.Show();
        }
        private void btnAgregar_Click(object sender, EventArgs e)
        {
            ...
            if (this.txtNroDocumento.Text.Trim() != "")
            {
                        ...
                        if (Convert.ToInt32(txtCantidad.Text) >= 0)
                        {
                            if (Convert.ToInt32(txtCantidad.Text) <= Convert.ToInt32(txtStock.Text))
                            {
                                if (txtIVA.Text.Trim() != "")
                                {
                                    ...
                                    Porcentaje = (Convert.ToDecimal(txtIVA.Text) / 100) + 1;
                                    SubTotal = ((Convert.ToDecimal(txtPVenta.Text) * Convert.ToInt32(txtCantidad.Text)) / Porcentaje);
                                    V.Igv = Math.Round(Convert.ToDecimal(SubTotal) * (Convert.ToDecimal(txtIVA.Text) / (100)), 2);
        ...
        private void LlenarGrilla()
        {
            ...
            dataGridView1.Rows.Add();
            dataGridView1.Rows.Add();
            dataGridView1.Rows[lst.Count + 1].Cells[3].Value = "SUB-TOTAL  S/IVA.";
        ...
        private void GuardarVenta()
        {
            ...
            if (Convert.ToString(dataGridView1.CurrentRow.Cells[2].Value) != "")
            {
                ...
                Ventas.IdUsuario = Program.IdUsuarioLogueado;
